...
namespace TestTask
{
    public class Program
    {
        ...
        private static IList<LetterStats> FillSingleLetterStats(IReadOnlyStream stream)
        {
            ...
            LetterStats letter = new LetterStats();         
            while (!stream.IsEof)
            {
        ...
        private static void RemoveCharStatsByType(IList<LetterStats> letters, CharType charType)
        {
            ...
            switch (charType)
            {
                case CharType.Consonants:
                    ...
                    {
                        string gfgf = letters[i].Letter.ToString().ToLower().Substring(1);
                        
                        if (!vowels.Contains(letters[i].Letter.ToString().ToLower().Substring(1)))
                        {
                ...
                case CharType.Vowel:
                    for (int i = 0; i < letters.Count; i++)
                    {
                        letters[i].Letter.ToLower();
                        if (vowels.Contains(letters[i].Letter.ToString().ToLower()))
                        {
                            letters.Remove(letters[i]);
                            i--;
                        }
